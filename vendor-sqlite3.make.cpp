...
namespace Main {
	...
	int Application::main(int cmdN, char *cmdS[]) {
		...
		String softwareName = "sqlite3";
		String softwareVersion = "3.31.1";
		...
		cFiles.push(workspaceSource + "/sqlite3.c");
		...
		cppDefine[0] = "SQLITE_ENABLE_FTS4";
		cppDefine[1] = "SQLITE_ENABLE_RTREE";
		cppDefine[2] = "SQLITE_ENABLE_COLUMN_METADATA";
		cppDefine[3] = "SQLITE_ENABLE_FTS5";
		...
		if(!Compiler::makeCToLib(
				...
				cppDefine, incPath, incFiles, cFiles,
				rcDefine, incPathRC, rcFiles,
				workspacePath + "/port/sqlite3.def",
				libDependencyPath, libDependency,
				libVersion,
				8,
				true
		...
		cFiles.empty();
		cFiles.push(workspaceSource + "/shell.c");

		libDependencyPath[0] = "lib";
		libDependency[0] = "libsqlite3";
		...
		if(doRelease) {
			...
			if(!Compiler::copyDllToFolder(binPath + "/libsqlite3", pathRelease, "")) {
				return 1;
			};

			if(!Compiler::copyExeToFolder(binPath + "/sqlite3", pathRelease)) {
				return 1;
			};
		...
		if(doInstall) {
			if(!Compiler::copyLibToFolder(libPath + "/libsqlite3.static", pathRepository + "/lib")) {
				return 1;
			};
			if(!Compiler::copyDllToFolderWithLib(
					libPath + "/libsqlite3", pathRepository + "/lib",
					binPath + "/libsqlite3", pathRepository + "/bin",
					libVersion)) {
				return 1;
			};
			if(!Compiler::copyExeToFolder(binPath + "/sqlite3", pathRepository + "/bin")) {
				return 1;
			};
			if(!Shell::copyFile(workspaceSource + "/sqlite3.h", pathRepository + "/include/sqlite3.h")) {
				return 1;
			};
			if(!Shell::copyFile(workspaceSource + "/sqlite3ext.h", pathRepository + "/include/sqlite3ext.h")) {
				return 1;
			};
		};
